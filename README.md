...
Choose one AI entry, enter only the connection details it needs, and run its readiness check. For a CLI entry, **Authentication and model** can inspect the CLI-owned sign-in, start or cancel the CLI's normal authorization flow, and load the models, model-specific reasoning efforts, and supported inference speeds currently advertised by that CLI. Local Reader App keeps the active CLI entry, catalog-bound model/effort/speed selection, readiness display, and repository-specific conversation state in tab-scoped browser session storage so they survive a reload in the same tab. It does not put CLI credentials, transient authorization values, the HTTP session token, executable details, or server readiness attestations there. If tab session storage is unavailable, invalid, or over its bounded limit, Local Reader App discards the saved payload, continues with current-page memory, and warns that a reload may clear that state. Persistent CLI sign-in remains in storage managed by the CLI itself, outside Local Reader App. AI API and Local AI values retain their existing current-page-only behavior.
...
| CLI AI conversation and draft | Tab-scoped browser session storage, separated by repository ID | Kept in the same tab; an in-flight request is restored as interrupted, and attachments are cleared |
| Active CLI entry, CLI model/effort/speed selection, and readiness display | Tab-scoped browser session storage; restored only after the live setup/catalog validates the selection | Kept in the same tab |
| Credential values entered into Local Reader App for AI API or Local AI | Current page memory only | Cleared |
...
3. Open **Settings** > **AI Chat** and select **Set active** for the installed CLI. Local Reader App re-inspects setup, binds the catalog's explicit default model/effort and Standard inference speed on the first activation, and runs **CLI Readiness** automatically. If the entry is already active, **Check again** in **Authentication and model** repeats that complete sequence.
...
5. Review the selected model, reasoning effort, and inference speed. **Standard** is the default. **Fast** appears only when the current model advertises support and may consume provider usage at a higher rate. Choosing another model binds that model's declared effort and speed defaults instead of carrying an unsupported combination across models. The catalog's explicit default model is used only when no prior selection exists. Local Reader App has no fixed model fallback and does not silently replace a prior model that disappears after a catalog or CLI change.
6. Confirm that **CLI Readiness** succeeded, then send the request from AI Chat. Changing to another catalog-valid model, reasoning effort, or speed keeps the fresh readiness result; the server still validates the complete selection against the current catalog immediately before each run. Switching the Current repo keeps the browser readiness display and selection, but never transfers a server readiness attestation from one repository to another. The next send reuses a valid attestation for the target repository or automatically repeats the server checks. A CLI identity, authentication, catalog, or setup-generation change still fails closed and may require **Check readiness** or **Check again**.
...
Inference speed is applied per request and never changes user-level CLI configuration. Codex maps **Standard** to its regular `flex` service tier and **Fast** to the advertised `fast` service tier with the session Fast feature enabled. Claude models expose **Fast** only when Agent SDK metadata reports `supportsFastMode`; Local Reader App passes `fastMode` inside its app-owned session settings, while **Standard** passes `false`. Speed and reasoning effort are independent selections.

Claude Code setup is currently a foundation-only integration in this release: the production adapter and Local HTTP contract, including Standard/Fast normalization, are present and covered by fake/static tests, but the maintainers did not use a Claude subscription to live-validate authentication, catalog loading, readiness, speed mode, or a model request. Treat live Claude compatibility as unverified until it is tested with an eligible account. Codex setup, catalog selection, and a normal send are the live validation gate for this change.
...
- Select **New chat** in the AI Chat-only header to clear the current repository's transcript, draft, retry state, attachments, and one-request context without clearing the active AI Entry, readiness display, or another repository's conversation.
- Switch repositories without clearing the active AI Entry or readiness display. Each repository has its own transcript and draft, which reappear when you return to that repository.
- Reload in the same browser tab to restore the active CLI entry, validated selection, readiness display, completed or interrupted repository conversations, and drafts. Uploaded attachments and active request handles are not restored.
...
- Choose the model, model-specific reasoning effort, and Standard or supported Fast inference speed advertised by the active Codex CLI or Claude Code CLI. The complete selection is shown beside the composer and is sent through the CLI's per-request arguments or app-owned session settings.

Only one AI run can use the same repository at a time, with up to four runs across the server. A CLI readiness lease is scoped to the selected repository/revision, entry, CLI version, catalog revision, server-owned setup generation, and inspected execution descriptor. Model, effort, and speed are not part of the lease key: switching among combinations that remain valid in that same catalog generation keeps readiness, while the server revalidates the requested combination before lease reuse and again before execution. Each catalog refresh, authentication change, update transition, or provider invalidation advances the generation and requires a new readiness result, even if the same catalog revision and selection reappear. Switching repositories never transfers a lease: the target repository's valid lease is reused, or sending automatically repeats the target-specific checks before the CLI starts. A stale setup response cannot overwrite a newer terminal state; an observed setup identity change also cancels an in-flight browser request. A failed renewal or stale selection stops before the CLI run. A same-tab reload restores only bounded browser display and conversation state, then rebinds the saved selection to the live setup/catalog; it does not restore credentials, an executable descriptor, an active operation, or a server lease. Restarting the server invalidates the old browser session and every lease. Reload or reopen the exact URL printed by the new server to obtain its new session; same-tab conversation state may remain, but the next send must pass the new server's validation.
...
- Select **Set active**, or select **Check again** in **Authentication and model** for the active entry. Local Reader App re-inspects setup, safely binds a usable catalog selection, and runs readiness. Complete **Sign in** if required; if a prior selection disappeared, choose a currently advertised model, reasoning effort, and speed explicitly.
...
- If the catalog changed, reselect a valid model/effort/speed combination. Local Reader App deliberately does not substitute another model for a stale selection.
- Confirm that the selected Current repo still exists and is writable by your account.
- A repository switch does not require manual activation or a manual readiness check; the next send validates the selected repository and creates or reuses only its own server attestation. Select **Check readiness** again after a CLI identity, authentication, catalog, or setup-generation change when the UI asks for it. A model/effort/speed change inside the same fresh catalog does not require another check. AI API and Local AI remain disabled **Coming soon** entries in this build.
...
4. Reload or reopen the exact URL printed by the new server, let the restored selection pass the new server's validation or run readiness again if requested, and only then retry. A same-tab reload keeps the bounded conversation and draft, but does not restore the old server lease or active run.
