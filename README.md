...
| AI entry state, credential values entered into Local Reader App, and CLI model/effort/speed selection | Current page memory only | Cleared |
...
6. Confirm that **CLI Readiness** succeeded, then send the request from AI Chat. Changing to another catalog-valid model, reasoning effort, or speed keeps the fresh readiness result; the server still validates the complete selection against the current catalog immediately before each run. Changing the Current repo, CLI identity, authentication, catalog, or setup generation invalidates readiness. Use **Check readiness** or **Check again** only after one of those execution boundaries changes or the readiness lease expires.
...
- Select **Check readiness** again after changing the repository, CLI identity, authentication, or catalog. A model/effort/speed change inside the same fresh catalog does not require another check. AI API and Local AI remain disabled **Coming soon** entries in this build.
