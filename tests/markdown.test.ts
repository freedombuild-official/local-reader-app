...
describe("renderMarkdown", () => {
  it("adds table scroll wrappers and code block toolbar controls without app-specific data names", () => {
    const source = ["# Doc", "", "![Local](assets/local.png)", "", "- [x] Done", "- [ ] Todo", "", "| A | B |", "| --- | --- |", "| 1 | 2 |", "", "> Quote", "", "```ts", "console.log(1)", "```"].join("\n");
    const { html } = renderMarkdown(source, { repoId: "docs", currentPath: "README.md", revision: "revision-test" });
    ...
    expect(html).toContain("/api/image?repo=docs&amp;path=assets%2Flocal.png&amp;revision=revision-test");
