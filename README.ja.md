...
- Git が現在 ignore と判定する可視ファイルとフォルダは、名前、アイコン、展開用の矢印を控えめな灰色で表示します。ignore 規則に一致していても追跡済みのファイルは灰色にせず、リポジトリ設定の `excludes` にあるパスは引き続き非表示です。
...
Git がインストール済みか、登録フォルダが Git の作業ツリーか、そこで `git status` が動くか確認します。ignore 対象の灰色表示は、Git 自身の `git check-ignore` の結果に従い、追跡済みファイルは意図的に灰色にしません。どちらかの Git コマンドが失敗しても、取得できない Git 表示を省略して基本的なファイル閲覧は継続します。
